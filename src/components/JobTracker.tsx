...
const JobTracker: React.FC = () => {
  ...
  // const editSchedule = (jobIndex: number) => {
  //   const scheduleToEdit = jobs[jobIndex].schedules;
  //   if (scheduleToEdit) {
  ...
  //   if (scheduleToEdit) {
  //     setEditedSchedule(scheduleToEdit);
  //     setSelectedJobIndex(jobIndex);
  //     setShowEditScheduleModal(true);
  //   }
  // };
  // const saveEditedSchedule = (selectedDate: string, startTime: string, endTime: string) => {
  //   if (selectedJobIndex !== null && editedSchedule) {
  //     const updatedJobs = [...jobs];
  //     updatedJobs[selectedJobIndex] = {
  //       ...updatedJobs[selectedJobIndex],
  //       schedules: updatedJobs[selectedJobIndex].schedules?.map((schedule) =>
  //         schedule.date === editedSchedule.date
  //           ? { date: selectedDate, startTime, endTime }
  //           : schedule
  //       ),
  //     };
  //     setJobs(updatedJobs);
  //     localStorage.setItem('jobs', JSON.stringify(updatedJobs)); // Save updated jobs to local storage
  //   }
  //   setShowEditScheduleModal(false);
  //   setEditedSchedule(null); // Clear the edited schedule state
  // };
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {jobs.map((job, index) => {
          console.log("3. creaing index from job tracker, in map", index)
          return (
            <JobCard
              key={index}
              job={job}
...
{/* <EditScheduleModal
  ...
  editedSchedule={editedSchedule}
  saveEditedSchedule={saveEditedSchedule}
  existingSchedules={selectedJobIndex !== null ? jobs[selectedJobIndex].schedules || [] : []} // Check if selectedJobIndex is not null
/> */}
